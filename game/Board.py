...
def generate_catan_board():
    ...
    """
    # 1) Build and shuffle the resource pool
    resources = []
    for res, count in RESOURCE_ALLOTMENT.items():
        resources.extend([res] * count)
    random.shuffle(resources)

    # 2) Shuffle the number tokens (skip desert)
    ...
    for tile_id, res in enumerate(resources, start=1):
        if res == "desert":
            # Desert: no token, no dots, robber starts here
            board[tile_id] = [res, None, 0, True]
        else:
            # Assign next token and its dot count
            number = tokens.pop()
            board[tile_id] = [res, number, DOTS[number], False]
...
#    for tile_id, info in sorted(board.items()):
#        resource, token, dots, robber = info
#        print(f"Tile {tile_id:2d}: {resource:6s} | Token: {token!s:2s} | Dots: {dots} | Robber: {robber}")